import { ButtonInteraction, Collection, CommandInteraction, InteractionCollector, Message, MessageActionRow, MessageButton, MessageEmbed } from "discord.js";

type OnCollectCallback = (collector: InteractionCollector<ButtonInteraction>, inter: ButtonInteraction) => Promise<void> | void;
type OnEndCallback = (coll: Collection<string, ButtonInteraction>) => Promise<void> | void;
...
export default class PaginatedEmbed {
    ...
    public async send() {
        ...
        const row = new MessageActionRow()
            .addComponents(this.prev, this.next, ...this.otherButtons)
        ...
        const filter = (i: ButtonInteraction) => {
            ...
            return i.customId === this.prev.customId ||
                i.customId === this.next.customId ||
                otherCustomIds.includes(i.customId);
        }

        const collector = msg.createMessageComponentCollector({ filter, componentType: "BUTTON", time: this.timeout });

        collector.on("collect", async inter => {
            await inter.deferUpdate();
            if (inter.customId === this.next.customId || inter.customId === this.prev.customId) {
    ...
    public static generateFromTemplate<T>(options: PaginatedTemplateOptions<T>) {
        const { entries, perPage, perPageCallback, perItemCallback, base } = options;
